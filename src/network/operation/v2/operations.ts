...
export async function getOperationV2Operations(accessToken: string, contractNumber: string, paginationId?: string): Promise<{
  operations: Array<Operation>
  pagination_id: string
}> {
  let uri = `${BASE_URL}/operation/v2/operations/${contractNumber}`;

  if (paginationId)
    paginationId += `?since_id=${paginationId}`;
